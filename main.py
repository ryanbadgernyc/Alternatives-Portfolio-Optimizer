...
# --- SESSION STATE SETUP ---
if 'assumptions_loaded' not in st.session_state:
    st.session_state['assumptions_loaded'] = False

# --- IMPORT ASSUMPTIONS ---
st.subheader('Import Previously Saved Assumptions')
uploaded_file = st.file_uploader("Choose a CSV file with assumptions", type=["csv"])

if uploaded_file is not None:
    df_uploaded = pd.read_csv(uploaded_file)
    assets = df_uploaded['Asset'].tolist()
    expected_returns = df_uploaded['Expected Return (%)'].tolist()
    volatilities = df_uploaded['Volatility (%)'].tolist()
    max_weights = df_uploaded['Max Weight (%)'].tolist()
    st.session_state['assumptions_loaded'] = True
else:
    # Asset names
    assets = st.text_input('Enter Asset Names (comma-separated)', 'Stocks, Bonds, Real Estate').split(',')
...
for idx, col in enumerate(cols):
    with col:
        if st.session_state['assumptions_loaded']:
            expected_returns.append(st.number_input(f'{assets[idx]} Return (%)', value=float(expected_returns[idx])))
            volatilities.append(st.number_input(f'{assets[idx]} Volatility (%)', value=float(volatilities[idx])))
            max_weights.append(st.number_input(f'{assets[idx]} Max Weight (%)', value=float(max_weights[idx])))
        else:
            expected_returns.append(st.slider(f'{assets[idx]} Return (%)', -5.0, 20.0, 6.0, step=0.1))
            volatilities.append(st.slider(f'{assets[idx]} Volatility (%)', 1.0, 50.0, 15.0, step=0.5))
            max_weights.append(st.slider(f'{assets[idx]} Max Weight (%)', 0.0, 100.0, 100.0, step=1.0))
...
def correlation_input():
    ...
    for i in range(num_assets):
        for j in range(i+1, num_assets):
            cor_val = st.slider(f'Correlation {assets[i]} - {assets[j]}', -1.0, 1.0, 0.2, step=0.05)
...
if st.button('Generate Efficient Frontier'):
    ...
    if len(results) > 0:
        ...
        st.warning('No portfolios met the maximum weight constraints. Try adjusting them.')
    ...
    st.subheader('Download Current Assumptions')
    assumptions_df = pd.DataFrame({
        'Asset': assets,
        'Expected Return (%)': [r for r in expected_returns],
        'Volatility (%)': [v for v in volatilities],
        'Max Weight (%)': [w for w in max_weights]
    })
